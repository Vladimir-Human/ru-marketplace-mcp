...
it. Only the outbound fetch unwraps it (the connector's ``_proxy()``). The
mp_auth token was already a SecretStr; this file pins the proxy alongside it.
"""
...
from mpstats_connector.settings import MPStatsSettings
...
def test_the_proxy_secret_never_appears_in_settings_dumps():
    settings = MPStatsSettings(proxy=SecretStr(SECRET_PROXY))
