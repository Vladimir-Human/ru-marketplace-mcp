...
class MpStatsItem(BaseModel):
    ...
    non-zero ``count_graph`` cell. Both are ``None`` when their graph is empty,
    so a delisted item never reports a false price/stock. ``stock_now`` is ``0``
    for a non-empty all-zero count graph — a real out-of-stock reading, distinct
    from "no data" — while an all-zero price graph stays ``None``.
    """
...
class MpStatsNoResultsResponse(BaseModel):
    """Structured ``no_results`` payload for SKUs MPStats has no analytics on.

    Not wired into the tools today — ``mpstats_item`` surfaces missing SKUs via
    ``meta.warnings`` instead. Kept as the documented shape of the upstream
    no-results answer for a future tool that needs it as a value.
    """
