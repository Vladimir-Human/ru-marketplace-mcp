...
class _PoliteGate:
    """Adapter exposing the module-level polite gate as a ``RateLimiter``.

    Mirrors the WB connector's pattern: the gate is module state tests can
    monkeypatch directly, while ``get_text_budgeted`` gets a limiter to re-enter
    between retries.
    """
...
async def _polite_wait() -> None:
    """Space consecutive requests by ``min_gap`` — shared, quota-billed endpoint."""
...
def _cookie_header() -> str:
    ...
    """
    token = (_settings.mp_auth or "").strip()
...
async def _call(body: dict[str, Any], *, label: str) -> dict[str, Any]:
    ...
    if cached is not None:
        ...
        try:
            ...
            raise_tool_error(TransportDownError(str(exc), provider="mpstats"))

        if err:
            log_event(f"{label}.network_error", error=_redact(err))
            raise_tool_error(TransportDownError(err, provider="mpstats"))
        if status_code == 429:
        ...
        if status_code == 200 and text is not None:
...
def _parse_item_entry(raw: Any, place: str) -> MpStatsItem | None:
    ...
    if stock_now is None and count_graph:
        # _last_nonzero returned None because every cell was 0 -> genuinely zero
        # stock now is a real reading here, distinct from "no data" (empty graph).
        stock_now = 0
    return MpStatsItem(
