...
Новый коннектор MPStats — первый платный источник в проекте. До этого все
серверы работали по анонимным scrape-эндпоинтам без ключей; MPStats качественно
иной: это аналитический слой поверх Ozon/Wildberries с квотой и аккаунтом.

### Добавлено

**Новый сервер `mpstats-mcp` (3 инструмента)**
- `mpstats_item(skus, place, oz_fbs=True)` — аналитика продаж/цены/остатков за 30
  дней по до 100 SKU Ozon или Wildberries: 4 почасовых графика (заказы, цены,
...
- `mpstats_warehouses(skus, place)` — остатки по складам: FBS (склад продавца) и
  FBO (склад маркетплейса, свернутый в total из списка по складам), плюс
  `last_update` апстрима.
- `mpstats_selfcheck()` — три-стейт канарейка: `success` / `drift_detected` /
  `inconclusive`. Отсутствие токена и транспортные сбои — `inconclusive`, не
  `drift`, чтобы не гнать мейнтейнера искать дрейф схемы, которого не было.

**Авторизация.** Эндпоинт `POST plugin.mpstats.io/pluginapi` (RPC-стиль: метод в
поле `Request`, параметры — `Sku`/`Place`/`ozFBS`). Авторизация — одна cookie
`mp_auth` (JWT из залогиненной сессии плагина в mpstats.io), задаётся через
`MPSTATS_MP_AUTH`. Проверено живьём в июле 2026: холодный replay с одним только
`mp_auth` отвечает 200 с данными; без него тело `{"code":403,"message":"Unauthorized"}`
за HTTP 200 — этот внутренний код мапится в `auth_missing`, чтобы не выглядело
«нет данных» вместо отказа авторизации. Без токена сервер запускается чисто,
инструменты возвращают `auth_missing`. Токен — секрет платного аккаунта с квотой,
не логируется и не коммитится.

**Транспорт.** Tier 1 (httpx), но POST-only, поэтому вместо GET-only
`get_text_budgeted` из `mcp-core` в коннекторе локальный помощник
`_post_json_budgeted` с теми же инвариантами: байтовый кап, wall-clock бюджет,
классифицированные ошибки, ретраи только транспортных сбоев, вежливый гейт между
попытками. В `mcp-core` POST-путь не добавлен намеренно: POST нужен одному
коннектору, тянуть его в общий рантайм — плодить тестовую поверхность для всех.
...
ложный `0`. Ноль в ячейке — «нет данных за тот день», не «значение было нулевым».
`coerce_int`/`coerce_price` из `mcp_core.resilience` отказываются угадывать
неоднозначные формы (знак, диапазон цен) — `None` вместо правдоподобно-неправильного
числа. 30 офлайн-тестов покрывают парсеры, контракт ошибок и три-стейт selfcheck.
