...
anonymous catalog connectors in this workspace, this one needs a paid MPStats
account configured as the `MPSTATS_MP_AUTH` env var (the raw `mp_auth` JWT
cookie from a logged-in browser plugin session at mpstats.io).
...
- Graphs are length `days` (default 30), **oldest-first**; the final cell is
  "today". The current price/stock is the last **non-zero** cell, so a delisted
  item reports `None`, not a false `0.0` that would rank it as cheapest.
