...
uv run pytest -q -m "not live and not cdp"   # 1091 офлайн-тест, сеть не нужна
```
...
```bash
uv sync --all-packages
uv run pytest -q -m "not live and not cdp"    # 1091 офлайн-тест
...
ошибаются уверенно, поэтому проект устроен вокруг проверки: 1091 офлайн-тест,
...
cd ru-marketplace-mcp
uv sync --all-packages
uv run pytest -q -m "not live and not cdp"    # 1091 offline tests, no network needed
```
...
```bash
uv sync --all-packages
uv run pytest -q -m "not live and not cdp"    # 1091 offline tests
...
are confidently wrong, so the project is arranged around verification: 1091 offline
