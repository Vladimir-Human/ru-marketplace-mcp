...
uv run pytest -q -m "not live and not cdp"   # 1082 офлайн-теста, сеть не нужна
```
...
```bash
uv sync --all-packages
uv run pytest -q -m "not live and not cdp"    # 1082 офлайн-теста
...
ошибаются уверенно, поэтому проект устроен вокруг проверки: 1082 офлайн-теста,
...
cd ru-marketplace-mcp
uv sync --all-packages
uv run pytest -q -m "not live and not cdp"    # 1082 offline tests, no network needed
```
...
```bash
uv sync --all-packages
uv run pytest -q -m "not live and not cdp"    # 1082 offline tests
...
are confidently wrong, so the project is arranged around verification: 1082 offline
