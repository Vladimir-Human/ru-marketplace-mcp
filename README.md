...
готовый блок для вставки с реальным путём к вашему checkout
...
неизвестное имя клиента (допустимы `claude`, `claude-code`, `cursor`) отклоняется с пояснением
...
`mcp-core` — общий рантайм, и навыка у него нет.
