...
uv run pytest -q -m "not live and not cdp"   # 1032 офлайн-теста, сеть не нужна
```
...
```bash
uv sync --all-packages
uv run pytest -q -m "not live and not cdp"    # 1032 офлайн-теста
...
ошибаются уверенно, поэтому проект устроен вокруг проверки: 1032 офлайн-теста,
...
cd ru-marketplace-mcp
uv sync --all-packages
uv run pytest -q -m "not live and not cdp"    # 1032 offline tests, no network needed
```
...
```bash
uv sync --all-packages
uv run pytest -q -m "not live and not cdp"    # 1032 offline tests
...
are confidently wrong, so the project is arranged around verification: 1032 offline
